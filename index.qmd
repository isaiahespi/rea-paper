...
ggpubr::ggarrange(
  ...
  # add figure footnote annotation
  patchwork::plot_annotation(
    caption = str_wrap("Missing values removed. Displays percent proportions of responses by experiment condition and combined percentage proportions for two categories.",
               width = 85), theme = theme(text = element_text(family = "Palatino Linotype")))
...
@fig-coef1 displays standard difference estimates of confidence in elections by treatment condition (control condition as reference). @fig-coef1-1 displays two models for confidence in elections in Maricopa County, AZ, and one's local area by treatment condition; @fig-coef1-2 displays four models for trust and distrust in elections in AZ or one's local area by treatment condition. These figures merely illustrate the effect of the treatment with respect to the relationship between trust and distrust; ultimately confidence improves. 
...
Additionally, these results cannot give any indication whether confidence in the electoral process or its administration writ large is higher among those who, while voting in person, interacted with military veterans who were working as election staff or volunteers. In other words, this research can provide no indication that having veterans on staff at election sites or individual interaction with veterans while voting has any influence on voter confidence in the post-election period. 

All that can be said from the results presented is that public announcement of efforts to recruit veterans to work as election workers results in a small boost in confidence in elections held in places where confidence in elections administration is lower relative to the confidence in elections within one's local area. 
