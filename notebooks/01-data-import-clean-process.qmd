...
raw_spss <- haven::read_sav("data-raw/vmf_election_worker_recruitment_survey_raw_spss_sanitized_20241029_T12.02.sav")
...
A data dictionary contains metadata about the data. The `labelled::generate_dictionary` function is used to create a data dictionary (alternatively, use the `data_dict()` function from R package [surveytoolbox](https://martinctc.github.io/surveytoolbox/)). This function produces a dataframe into the R environment with the number of observations (rows) equal to number of variables (columns) in the original data set. For ease, I prefer to save the codebook/data dictionary as a `.csv` file. In order to do so, ensure that the function `labelled::convert_list_columns_to_character()` is run on the data dictionary which allows you to save the data dictionary as a `.csv` file. Alternatively, the
function `surveytoolbox::data_dict()` permits saving as a .csv file without the
additional step. 
...
The data dictionary is the same as a codebook, but I also wanted to have a codebook that contained **only** the survey questions, variable labels (i.e., question text), responses (value labels), and associated values. So the saved `codebook` contains only the survey questions and the two grouping variables (`qset` and `group`).
...
From the raw data set, there are `r dim(raw_spss)[1]` observations in the data set (each observation is a row). The survey was programmed in such a way to automatically drop participants who, 
