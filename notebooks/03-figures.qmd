...
```{r}
#| label: fig-itemcor
#| fig-cap: "Inter-item correlations between AZ and local area items"

# Inter-item correlations between AZ and Local items
# use the `easystats` correlation package
item.cor <- df |>
  dplyr::rename(
    trustvote_az     = q19,
    trustadmin_az    = q20,
...
layers <- correlation::visualisation_recipe(
  item.cor, labs = list(title = "Polychoric Item Correlation Matrix"))
layers

plot(layers) + theme(axis.text.x = element_text(angle = 45, vjust = 0.6))
